...
module.exports = {
  name: "teleportcuruleoncave",
  description: "teleportcuruleoncave .",
  category: "Pokemon Commands",
  args: false,
  usage: ["teleportcuruleoncave"],
  cooldown: 15 ,
  permissions: [],
   aliases: ["teleportcuruleoncave","teleportcuruleon cave"],
    execute: async (client,message, args, prefix, guild, color, channel) => {
     ...
     if (user.techwatch<= 0) return message.channel.send(`> ❌ You Dont Have A Tech Watch `); 
      ...
      let names = ["magikarp","poliwag","golduck","gyarados","lickitung","rhyhorn","rhydon","mewtwo"]
    ...
    await get(ab).then(async x => {
       ...
       await spawn.save()
    user.techcrystals = user.techcrystals + 17
    await user.save();
