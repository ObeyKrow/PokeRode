...
module.exports = {
  name: "teleportseafoamisland",
  description: "teleportseafoamisland .",
  category: "Pokemon Commands",
  args: false,
  usage: ["teleportseafoamisland"],
  cooldown: 15 ,
  permissions: [],
   aliases: ["teleportseafoamisland","teleportseafoam island"],
    execute: async (client,message, args, prefix, guild, color, channel) => {
     ...
     if (user.techwatch<= 0) return message.channel.send(`> ❌ You Dont Have A Tech Watch `); 
      ...
      let names = ["magikarp","gyarados","slowpoke","slowbro","seel","dewgong","tentacool","tentacruel","articuno"]
    ...
    await get(ab).then(async x => {
       ...
       await spawn.save()
    user.techcrystals = user.techcrystals + 5
    await user.save();
