...
module.exports = {
  name: "teleportkyuremcave",
  description: "teleportkyuremcave .",
  category: "Pokemon Commands",
  args: false,
  usage: ["teleportkyuremcave"],
  cooldown: 15 ,
  permissions: [],
   aliases: ["teleport kyuremcave"],
    execute: async (client,message, args, prefix, guild, color, channel) => {
     ...
     if (user.techwatch<= 0) return message.channel.send(`> ❌ You Dont Have A Tech Watch `); 
      ...
      let names = ["kyurem","kingdra","glaceon","eevee","horsea","seadra","smoochum","jynx"]
    ...
    await get(ab).then(async x => {
       ...
       await spawn.save()
    user.techcrystals = user.techcrystals + 9
    await user.save();
