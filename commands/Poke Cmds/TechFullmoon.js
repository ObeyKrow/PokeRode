...
module.exports = {
  name: "tteleportfullmoon",
  description: "teleportfullmoon .",
  category: "Pokemon Commands",
  args: false,
  usage: ["teleportfullmoon"],
  cooldown: 15 ,
  permissions: [],
   aliases: ["teleport fullmoon","teleport fullmoonisland"],
    execute: async (client,message, args, prefix, guild, color, channel) => {
     ...
     if (user.techwatch<= 0) return message.channel.send(`> ❌ You Dont Have A Tech Watch `); 
      ...
      let names = ["cresselia","clefairy","clefable","clefa","igglubuff","jigglypuff","wigglytuff","chansey"]
    ...
    await get(ab).then(async x => {
       ...
       await spawn.save()
    user.techcrystals = user.techcrystals + 22
    await user.save();
