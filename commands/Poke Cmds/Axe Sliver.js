...
module.exports = {
  name: "usesliveraxe",
  description: "Use A sliverAxe .",
  category: "Pokemon Commands",
  args: false,
  usage: ["usesliveraxe"],
  cooldown: 13 ,
  permissions: [],
   aliases: ["usesliveraxe"],
    execute: async (client,message, args, prefix, guild, color, channel) => {

       let user = await User.findOne({id: message.author.id})

     if (user.sliveraxe<= 0) return message.channel.send(`> ❌ You need 1  Sliver axe to cut trees you currently have ${user.sliveraxe} you can get buy it from p!axeshop. `); 
