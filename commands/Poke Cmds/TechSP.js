...
module.exports = {
  name: "teleportskypillar",
  description: "teleportskypillar .",
  category: "Pokemon Commands",
  args: false,
  usage: ["teleportskypillar"],
  cooldown: 15 ,
  permissions: [],
   aliases: ["teleportskypillar","teleportsky pillar"],
    execute: async (client,message, args, prefix, guild, color, channel) => {
     ...
     if (user.techwatch<= 0) return message.channel.send(`> ❌ You Dont Have A Tech Watch `); 
      ...
      let names = ["mawile","crobat","sableye","swablu","altaria","zubat","golbat","raquaza"]
    ...
    await get(ab).then(async x => {
       ...
       await spawn.save()
    user.techcrystals = user.techcrystals + 25
    await user.save();
