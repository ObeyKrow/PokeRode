...
module.exports = {
  name: "usewoodenaxe",
  description: "Use A wooden Axe .",
  category: "Pokemon Commands",
  args: false,
  usage: ["usewoodenaxe"],
  cooldown: 15 ,
  permissions: [],
   aliases: ["usewoodenaxe"],
    execute: async (client,message, args, prefix, guild, color, channel) => {

       let user = await User.findOne({id: message.author.id})

     if (user.woodaxe<= 0) return message.channel.send(`> ❌ You need 1 Wooden axe to cut trees you currently have ${user.axe} you can get buy it from p!axeshop. `); 
