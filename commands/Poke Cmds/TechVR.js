...
module.exports = {
  name: "teleportvictoryroad",
  description: "teleportvictoryroad .",
  category: "Pokemon Commands",
  args: false,
  usage: ["teleportvictoryroad"],
  cooldown: 15 ,
  permissions: [],
   aliases: ["teleportvictoryroad","teleportvictory road"],
    execute: async (client,message, args, prefix, guild, color, channel) => {
     ...
     if (user.techwatch<= 0) return message.channel.send(`> ❌ You Dont Have A Tech Watch `); 
      ...
      let names = ["graveler","machamp","machop","machoke","golem","geodude","onix","moltres"]
    ...
    await get(ab).then(async x => {
       ...
       await spawn.save()
    user.techcrystals = user.techcrystals + Math.random()
    await user.save();
