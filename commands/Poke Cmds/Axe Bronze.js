...
module.exports = {
  name: "usebronzeaxe",
  description: "Use A bronze Axe .",
  category: "Pokemon Commands",
  args: false,
  usage: ["usebronzeaxe"],
  cooldown: 18 ,
  permissions: [],
   aliases: ["usebronzeaxe"],
    execute: async (client,message, args, prefix, guild, color, channel) => {

       let user = await User.findOne({id: message.author.id})

     if (user.bronzeaxe<= 0) return message.channel.send(`> ❌ You need 1 Bronze axe to cut trees you currently have ${user.bronzeaxe} you can get buy it from p!axeshop. `); 
