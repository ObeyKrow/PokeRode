  ...
  permissions: [],
   aliases: ["useaxe"],
    execute: async (client,message, args, prefix, guild, color, channel) => {
     ...
     if (user.axe<= 0) return message.channel.send(`> ❌ You need 1 axe to cut trees you currently have ${user.axe} you can get buy it from jake. `); 
      ...
      let names = ["sudoowoodo","trevenant","bonsly","exeggutor","grotle","torterra"]
    ...
    await get(ab).then(async x => {
       ...
       await spawn.save()
    user.wood = user.wood + Math.random()
    await user.save();
