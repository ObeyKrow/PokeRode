...
module.exports = {
  name: "teleportcaveoforigin",
  description: "teleportcaveoforigin.",
  category: "Pokemon Commands",
  args: false,
  usage: ["teleportcaveoforigin",],
  cooldown: 15 ,
  permissions: [],
   aliases: ["teleport caveoforigin"],
    execute: async (client,message, args, prefix, guild, color, channel) => {
     ...
     if (user.techwatch<= 0) return message.channel.send(`> ❌ You Dont Have A Tech Watch `); 
      ...
      let names = ["rhydon","rhyhorn","seel","dewgong","lapras","charizard","groudon ","kyogre"]
    ...
    await get(ab).then(async x => {
       ...
       await spawn.save()
    user.techcrystals = user.techcrystals + 20
    await user.save();
