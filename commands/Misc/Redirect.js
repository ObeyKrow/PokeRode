...
module.exports = {
    ...
    if(!message.member.hasPermission('MANAGE_MESSAGES')) return message.channel.send('> **You need permission to use this command**')
    ...
    if(!nchannel) return message.channel.send(`> Correct usage: **${nguild.prefix || client.config.prefix}redirect <mentionchannel/id/name>**`);
      ...
      await nguild.save();
      return message.channel.send(`> ${config.yes} **PokeRode will now redirect all spawns to <#${nguild.spawnchannel}> To allow spawns in all channels again, type **\`${prefix}redirect disable.\``);
  }
}   